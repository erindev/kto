<?php
	...
	$target_dir = "adventure_uploads/"; // 올릴파일이름잡기
	$target_file = $target_dir . basename($_FILES["fileToUpload"]["name"]); // 전체 경로: uploads/filename
	$uploadOk = 1;
	$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);


	$sql = "insert into adventure
			(vtitle, vcity, vcont, vimage,vdate)
			values ('$vtitle','$vcity','$vcont','$target_file',now())";

	$result = mysqli_query($con, $sql) or die (mysqli_error($con));

	if ( $result ) { echo "<script>alert('[어드벤처]글쓰기가 성공했습니다');</script>"; } //07

	$sqlno = "select * from adventure where 'vtitle'='$vtitle'";
...
echo "<meta http-equiv='refresh' content='0; URL=../adventure.php'/>";
