<?php
	...
	$target_dir = "nature_uploads/"; // 올릴파일이름잡기
	$target_file = $target_dir . basename($_FILES["fileToUpload"]["name"]); // 전체 경로: uploads/filename
	$uploadOk = 1;
	$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);


	$sql = "insert into nature
			(atitle, acity, acont, aimage,adate)
			values ('$atitle','$acity','$acont','$target_file',now())";

	$result = mysqli_query($con, $sql) or die (mysqli_error($con));

	if ( $result ) { echo "<script>alert('[자연]글쓰기가 성공했습니다');</script>"; } //07

	$sqlno = "select * from nature where 'atitle'='$atitle'";
...
echo "<meta http-equiv='refresh' content='0; URL=../nature.php'/>";
