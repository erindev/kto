<?php
	...
	$etitle = $_POST['etitle'];
	$econt = $_POST['econt'];
	$edate = $_POST['edate1'] . " ~ " . $_POST['edate2'];
	$esubcont = $_POST['esubcont'];

	$target_dir = "event_uploads/"; // 올릴파일이름잡기
	...
	$sql = "insert into event
			(etitle,econt,edate,esubcont,eimage)
			values ('$etitle','$econt','$edate','$esubcont','$target_file')";

	$result = mysqli_query($con, $sql) or die (mysqli_error($con));

	if ( $result ) { echo "<script>alert('[이벤트]글쓰기가 성공했습니다');</script>"; } //07

	$sqlno = "select * from event where 'etitle'='$etitle'";
