...
//		} else {
		...
		$sql= "update stay SET stitle='$title', scont='$content', sdate='$date' where sno='$no'";
		$result = mysqli_query($con, $sql) or die (mysqli_error($con));
		$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
		
		echo "<script>alert('수정완료');</script>";
		echo "<meta http-equiv='refresh' content='0; url=./stay_detail.php?no=$no'/>";
//	}
}
			...
			<div class="contBody">
				...
				<div class="contArea">
					...
					if ( $_SESSION['userid'] == "admin" ){ ?>
							<li><a href="./stay_editform.php?no=<?=$detail?>">[수정하기]</a></li>
							<li><a href="./stay_deleteform.php?no=<?=$detail?>">[삭제하기]</a></li>
